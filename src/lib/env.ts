  ...
  .superRefine((env, ctx) => {
    ...
    if (isProductionRuntime && !hasDatabaseUrl) {
      ctx.addIssue({
        ...
        path: ["DATABASE_URL"],
        message:
          "DATABASE_URL, POSTGRES_URL, POSTGRES_PRISMA_URL, or POSTGRES_URL_NON_POOLING is required in production runtime.",
      });
    }
...
export function getDatabaseUrl(): string {
  ...
  if (isProductionRuntime) {
    throw new Error(
      "DATABASE_URL, POSTGRES_URL, or POSTGRES_PRISMA_URL environment variable is required in production",
    );
  }
