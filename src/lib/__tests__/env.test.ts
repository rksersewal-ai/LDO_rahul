  ...
  describe('validateEnv', () => {
    ...
    describe('production runtime requirements', () => {
      ...
      it('throws error for missing DATABASE_URL or POSTGRES_URL', async () => {
        const { validateEnv } = await import('../env');
        expect(() => validateEnv()).toThrow(/DATABASE_URL or POSTGRES_URL is required in production runtime/);
      });
      ...
      it('throws error for missing AUTH_SECRET or NEXTAUTH_SECRET', async () => {
        ...
        const { validateEnv } = await import('../env');
        expect(() => validateEnv()).toThrow(/AUTH_SECRET or NEXTAUTH_SECRET is required in production runtime/);
      });
      ...
      it('throws error if AUTH_SECRET uses placeholder', async () => {
        ...
        const { validateEnv } = await import('../env');
        expect(() => validateEnv()).toThrow(/AUTH_SECRET must not use the example or development placeholder value/);
      });
